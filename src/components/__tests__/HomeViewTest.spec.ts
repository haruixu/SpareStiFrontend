...
vi.mock('animejs', () => ({
    default: {  // Ensuring the mock includes a 'default' export
        timeline: vi.fn(() => ({
...
describe('HomeView', () => {
    ...
    it('animates on challenge completion', async () => {
        const challenge = {title: 'Mat og Drikke', challengeType: 'SNACKS', saved: 100, target: 100, completion: 100};
    ...
    it('persists animated challenges to localStorage', async () => {
        const challenge = { title: 'Gaming', challengeType: 'GAMING', saved: 100, target: 100, completion: 100 };
    ...
    it('correctly computes currentGoal based on goals', async () => {
        wrapper.vm.goals = [
            { title: 'Vacation', saved: 500, target: 1500, description: 'Summer vacation', priority: 1, completion: 33 },
            { title: 'Camera', saved: 300, target: 800, description: 'New camera', priority: 2, completion: 37 }
        ];
        await wrapper.vm.$nextTick();
    ...
    it('responds to changes in challenges and updates animation states', async () => {
        wrapper.vm.challenges.push({ title: 'New Challenge', challengeType: 'LEARNING', saved: 50, target: 100, completion: 50 });
        await wrapper.vm.$nextTick();
    ...
    it('triggers animation on completion', async () => {
        const challenge = { title: 'Test Challenge', challengeType: 'TEST', saved: 100, target: 100, completion: 100 };
        await wrapper.vm.animateChallenge(challenge);
