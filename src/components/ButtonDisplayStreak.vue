<template>
    <div class="flex flex-col items-center absolute">
        ...
        <button @mouseover="display" @mouseleave="hide" class="bg-transparent delay-200 cursor-pointer">
        ...
        >
            ...
            <div class=" flex flex-col justify-evenly w-full h-full py-2 px-4 md:py-0 bg-white rounded-2xl border-4 border-green-300" >
                <span class=" text-xs md:text-2xl font-bold text-black"
                    >{{ currentStreak }}{{ currentStreak === 1 ? ' utfodring fullført' : ' utfodringer fullført' }} streak</span
                >
                ...
                <Countdown v-if="screenSize > 768 && currentStreak!>0" class="flex flex-row" countdownSize="1rem" labelSize=".5rem" mainColor="white" secondFlipColor="white" mainFlipBackgroundColor="#30ab0e" secondFlipBackgroundColor='#9af781' :labels="{ days: 'dager', hours: 'timer', minutes: 'min', seconds: 'sek', }" :deadlineISO="deadline"></Countdown>
                        ...
                        <div v-for="index in 6" :key="index" class="min-w-max mx-auto">
                            <div class="flex flex-col items-center">
                                <span class="text-black text-xs md:text-1xl font-bold"
                                    >{{ currentStreak! - ((currentStreak! % 7) - index) }}</span
                                >
